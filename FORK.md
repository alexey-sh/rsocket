...
| Module format | CJS only (`main`)       | **Dual ESM + CJS**                             | Build tool `tsc` → **tsdown** (rolldown); each package emits `index.{js,mjs,d.ts,d.mts}` + sourcemaps behind a package.json `exports` map + `sideEffects:false` |
